...
###############################################################################
# This script pre-processes the records by de-meaning, performing base-line
# corrections, and eliminates records with SNR < 3. This is the paralellized
# version of pre-process_data.py
###############################################################################
...
for index in subdata:
    ...
    for stn in stns:
        ...
        if (pind - nind)/samprate > 8:
            ...
            noise = 'pre_noise'
            duration = 6
        # elif (pind - nind)/samprate > 5 and (pind - nind)/samprate < 6:
        #     duration = 5
        # elif (pind - nind)/samprate > 4 and (pind - nind)/samprate < 5:
        #     duration = 4
        else:
            noise = 'coda_noise'
            duration = 8 
            # print(f'{orgt} {stn} unuseable')
        ...
        Noise_source.append(noise)
        SNR_window.append(duration)
        SNR_E_freq_list.append(SNR_E_freq)
        SNR_N_freq_list.append(SNR_N_freq)
        SNR_Z_freq_list.append(SNR_Z_freq)
        SNR_E_HFfreq_list.append(SNR_E_HFfreq)
        SNR_N_HFfreq_list.append(SNR_N_HFfreq)
        SNR_Z_HFfreq_list.append(SNR_Z_HFfreq)
        SNR_E_time_list.append(SNR_E_time)
        SNR_N_time_list.append(SNR_N_time)
        SNR_Z_time_list.append(SNR_Z_time)

        # Save processed records if SNR greater than limit
        if not path.exists(event.replace(f"{wf_dir.split('/')[-1]}",f'SNR_{SNR}')):
            makedirs(event.replace(f"{wf_dir.split('/')[-1]}",f'SNR_{SNR}'))
        if not path.exists(event.replace('filtered',f'SNR_{SNR}_time')):
            makedirs(event.replace(f"{wf_dir.split('/')[-1]}",f'SNR_{SNR}'))
        
        # Save records with SNR > 3
        if SNR_N_freq >= SNR and SNR_E_freq >= SNR and SNR_Z_freq >= SNR and SNR_N_HFfreq >= SNR and SNR_E_HFfreq >= SNR and SNR_Z_HFfreq >= SNR and SNR_N_time >= SNR and SNR_E_time >= SNR and SNR_Z_time >= SNR:
            
            trE = E_basecorr[0]
            filename = E_file.replace(f"{wf_dir.split('/')[-1]}",f'SNR_{SNR}')
            trE.write(filename, format='MSEED')
            
            trN = N_basecorr[0]
            filename = N_file.replace(f"{wf_dir.split('/')[-1]}",f'SNR_{SNR}')
            trN.write(filename, format='MSEED')
            
            trZ = Z_basecorr[0]
            filename = Z_file.replace(f"{wf_dir.split('/')[-1]}",f'SNR_{SNR}')
            trZ.write(filename, format='MSEED')
            
        
all_ind = comm.gather(rm_ind, root=0)
Noise_source_gath = comm.gather(Noise_source, root=0)
SNR_window_gath = comm.gather(SNR_window, root=0)
SNR_E_freq_list_gath = comm.gather(SNR_E_freq, root=0)
SNR_N_freq_list_gath = comm.gather(SNR_N_freq, root=0)
SNR_Z_freq_list_gath = comm.gather(SNR_Z_freq, root=0)
SNR_E_HFfreq_list_gath = comm.gather(SNR_E_HFfreq, root=0)
SNR_N_HFfreq_list_gath = comm.gather(SNR_N_HFfreq, root=0)
SNR_Z_HFfreq_list_gath = comm.gather(SNR_Z_HFfreq, root=0)
SNR_E_time_list_gath = comm.gather(SNR_E_time, root=0)
SNR_N_time_list_gath = comm.gather(SNR_N_time, root=0)
SNR_Z_time_list_gath = comm.gather(SNR_Z_time, root=0)

if rank==0:
    all_ind = [item for sublist in all_ind for item in sublist]
    Noise_source_gath = [item for sublist in Noise_source for item in sublist]
    SNR_window_gath = [item for sublist in all_ind for item in sublist]
    SNR_E_freq_list_gath = [item for sublist in all_ind for item in sublist]
    SNR_N_freq_list_gath = [item for sublist in all_ind for item in sublist]
    SNR_Z_freq_list_gath = [item for sublist in all_ind for item in sublist]
    SNR_E_HFfreq_list_gath = [item for sublist in all_ind for item in sublist]
    SNR_N_HFfreq_list_gath = [item for sublist in all_ind for item in sublist]
    SNR_Z_HFfreq_list_gath = [item for sublist in all_ind for item in sublist]
    SNR_E_time_list_gath = [item for sublist in all_ind for item in sublist]
    SNR_N_time_list_gath = [item for sublist in all_ind for item in sublist]
    SNR_Z_time_list_gath = [item for sublist in all_ind for item in sublist]
    
    print(f'Removing indices {all_ind}')

    new_catalog = catalog.drop(index=all_ind)
    new_catalog.to_csv('/Users/tnye/kappa/data/flatfiles/data_download/bay_2000-2021_SNR_irfilt.csv')
