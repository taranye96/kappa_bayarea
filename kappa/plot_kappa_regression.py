...
def plot_kappa(model_name,home_dir,df,min_events):
    ...
    if len(df)>=min_events:
        stn = df['Name'].iloc[0]
        mag = np.array(df['Mag'])
        repi = np.array(df['repi'])
        depth = np.array(df['Qdep'])
        ...
        # Get R^2
        residuals = kappa - linearFunc(repi, *a_fit)
        ...
        errorbar(repi, kappa, yerr=kappa_std, xerr=0, ecolor='gray', alpha=.25, lw=1, fmt='none', label='error')
        
        xlabel(r'$R_{epi}$ (km)', fontsize=12)
        ylabel('Kappa (s)', fontsize=12)
        title(f'{stn}: Kappa={round(inter,3)} s', fontsize=12)
        plot(x, fit, 'r', lw=1, label='best fit curve')
        scatter(repi, kappa, c='k', s=5)
        ax.grid(linestyle='--',alpha=0.25)
        ax.fill_between(x, fit_up, fit_dw, alpha=.25, label='95% Confidence Interval')
        ax.set_xlim(xmin=0)
        legend(loc='lower right',fontsize=8)
        plt.savefig(f'{figpath}/{stn}_regress_k0.png', dpi=300)
        plt.close()
